...
1. **A signed-URL endpoint on the coordinator.** `POST /api/datasets/upload-url` returning a one-shot URL (S3 PUT for the s3 backend, a local-only proxy for the file backend). This is in flight as a separate `docs/security/` § "Signed URLs for direct node→S3 transfer" item — same primitive, different consumer.
